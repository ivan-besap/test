...
public class Server {
    ...
    private JmsTemplate jmsTemplate = null; // Integración con Amazon MQ
    ...
    public Server(
        ...
        this.jmsTemplate = jmsTemplate; // Inyectamos JmsTemplate
    }
    ...
    public void registerSession(ISession session) {
        ...
        if (!sessions.containsKey(sessionUUID)) {
            ...
            logger.info("Sesión registrada: {}", sessionUUID);
        } else {
            logger.info("Sesión ya está registrada: {}", sessionUUID);
        }
    }
